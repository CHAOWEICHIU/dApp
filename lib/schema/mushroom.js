...
const MushroomPlayerType = new GraphQLObjectType({
  name: 'MushroomPlayerType',
  fields: {
...
const MushroomTeamType = new GraphQLObjectType({
  name: 'MushroomTeamType',
  fields: {
    score: { type: GraphQLFloat },
    players: { type: new GraphQLList(MushroomPlayerType) },
  },
})
...
const MushroomGameType = new GraphQLObjectType({
  ...
  fields: {
    ...
    teams: {
      type: new GraphQLList(MushroomTeamType),
      resolve: ({ id, teamsCount }) => Promise.all(_.range(0, Number(teamsCount)).map(teamIndex => gameWithTeam(id, teamIndex).call()))
        .then((teams) => {
          const data = teams.map(teamData => ({
            score: _.toNumber(teamData['0']),
            players: teamData['1'].map((p, pI) => ({
              level: _.toNumber(teamData['2'][pI]),
              address: p,
            })),
          }))
          return data
