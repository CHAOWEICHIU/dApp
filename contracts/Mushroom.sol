...
contract Mushroom {
    ...
    function registerUser() public {
        require(users[msg.sender].registerBlockheight == 0, "you have registed");
    ...
    function gameWithTeam(uint256 game_index, uint8 team_index) public view returns (
        uint256,    // score
        address[],  // team_members
        uint8[]    // levels
        ){
        uint256 score = mushroomGames.calcMushroom(game_index, team_index, block.number);

        (address[] memory addresses, uint8[] memory levels) = mushroomGames.getTeamMember(game_index, team_index);
...
library MushroomGameSystem {
    ...
    function getTeamMember(Data storage self, uint256 game_index, uint256 team_index) internal view returns (address[], uint8[]) {
        MushroomGame storage _m = self.mushroomGames[game_index];
        address[] memory addr = new address[](_m.gamer_count / _m.team_count);
        uint8[] memory level = new uint8[](_m.gamer_count / _m.team_count);
        uint8 idx = 0;
        for(uint8 i=0 ;i< _m.gamer_count; i++) {
            if(_m.team_member[_m.gamer[i]] == team_index) {
                addr[idx] = _m.gamer[i];
                level[idx]=_m.levels[_m.gamer[i]];
                idx += 1;
            }
        }
        return (addr, level);
    ...
    function getGameInfo(Data storage self, uint256 game_index) internal view returns (
            string,     // pending || started || aborted || ended
            address[],  // participants
            uint256,    // created_at_block
            uint256,    // game_start_at
        ...
        return (status, addr, _m.created_at_block, _m.pending_block, _m.game_start_at, _m.team_count);
    }
    
    function getBankerGame(Data storage self, address _banker) internal view returns (bool, uint256) {
    ...
    function newGame(
        Data storage self, address _banker,
    ...
    function newGameItem(
        Data storage self, uint256 game_index, string _name,
...
library LedgerSystem {
    ...
    function operation(
        ...
        bool _positive, string _desc) internal {
